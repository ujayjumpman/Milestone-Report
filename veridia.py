...
#         if green_dates_list:
#             latest_year = max(date.year for date in green_dates_list)
#             latest_year_dates = [d for d in green_dates_list if d.year == latest_year]
#         else:
#             latest_year_dates = []
        
#         logger.info(f"Found {green_cells_found} green cells | Extracted {len(latest_year_dates)} dates")
#         return latest_year_dates
...
#                     if green_dates:
#                         logger.info(f"Found {len(green_dates)} green dates (all months)")
            ...
            for month in self.quarter_months:
                ...
                if is_external:
                    ...
                    if month_done is not None:
                        ...
                        # Leave cumulative percentage BLANK (don't calculate)
                        # Show only the KRA target count (from targets)
