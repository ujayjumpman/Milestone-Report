...
def setup_dynamic_months_and_targets(tracker_date):
    ...
    global MONTHS, TRACKER_DATE, PROCESSING_MONTHS, T5_TARGET_CELLS, T7_TARGET_CELLS, T7_HARDCODED_VALUES
    
    TRACKER_DATE = tracker_date
    tracker_month = tracker_date.month
    tracker_year = tracker_date.year
    
    logger.info(f"=== SETTING UP DYNAMIC MONTHS ===")
    logger.info(f"Tracker date: {tracker_date.strftime('%d-%m-%Y')} (Month: {tracker_month})")
    
    # Calculate the months to work with
    # If tracker is from September (month 9), we want to process August (month 8)
    # If tracker is from October (month 10), we want to process September (month 9)
    
    # Generate 3 months ending with the month before tracker month
    ...
    # For processing, we typically want to show results for the last month only
    # But we'll include all months up to the target month for flexibility
    ...
    for month_name in MONTHS:
        ...
        # If the month is before the tracker month, include it for processing
        if month_num < tracker_month or (month_num > tracker_month and tracker_month <= 3):
            PROCESSING_MONTHS.append(month_name)
    
    # If no processing months determined by logic above, at least include the last month
    ...
    # Setup T5 target cells dynamically
    T5_TARGET_CELLS = {}
    base_rows = [23, 24, 25, 26]  # Starting rows for each activity
    ...
    # Setup T7 target cells dynamically  
    T7_TARGET_CELLS = {}
    base_rows = [30, 31, 32, 33]  # Starting rows for each activity
    units = ['Flats', 'Flats', 'Flats', 'Modules']
    ...
    # Setup T7 hardcoded values for the target month if needed
    if PROCESSING_MONTHS and PROCESSING_MONTHS[-1] in MONTHS:
        ...
        logger.info(f"Setup T7 hardcoded values for {target_month_name}")
...
def get_slab_targets_fixed_cells(cos):
    ...
    raw = download_file_bytes(cos, KRA_KEY)
    wb = load_workbook(filename=BytesIO(raw), data_only=True)
    sheet = wb["VeridiaTargets Till August 2025"]  # This sheet name might need to be dynamic too
    
    # Map months to columns - this might need to be made more flexible
    month_to_col = {
        "June": "B18", "July": "C18", "August": "D18",
        "September": "E18", "October": "F18", "November": "G18"  # Add more as needed
    }
...
def count_tower6_completed(wb):
    ...
    for row in TOWER6_ROWS:
        for col in TOWER6_COLS:
            ...
            if cell_date:
                month_name = cell_date.strftime("%B")
                if month_name in MONTHS:  # Check against dynamic months
                    fill = cell.fill
                    if fill.fill_type == "solid" and fill.start_color:
                        if fill.start_color.rgb == GREEN_HEX:
                            counts[month_name] += 1
...
def build_t6_milestone_dataframe(targets, completed):
    ...
    target_text = f"{total_target} Slabs ({', '.join(target_parts)})"

    row = {
        "Milestone": "Milestone-01",
        "Activity": "Slab Casting", 
        "Target Till August": target_text,  # Column name stays same for compatibility
        "Weightage": weightage,
        "Weighted Delay against Targets": "",
        "Total achieved": "",
        "Delay Reasons_June 2025": "",
...
def count_completed_activities_by_module_and_month(wb, sheet_name, activity_mapping):
    ...
    for row_idx, row in enumerate(sheet.iter_rows(min_row=2), start=2):
        try:
            ...
            # Check all activities in the mapping systematically
            for standard_name, variations in activity_mapping.items():
                if standard_name == "El- First Fix":
                    # Special comprehensive check for El- First Fix
                    activity_lower = activity_name.lower().strip()
...
def get_t5_targets_and_progress(cos):
    ...
    for i, activity in enumerate(T5_ACTIVITIES):
        ...
        for m in MONTHS:
            if m in prev_months:
                count_cumulative = sum(activity_counts[activity][month] for month in MONTHS if MONTHS.index(month) <= MONTHS.index(m))
                target_cumulative = sum(t5_targets[activity][month][0] for month in MONTHS if MONTHS.index(month) <= MONTHS.index(m))
        ...
        row["Target Till August"] = f"{int(total_target)} {unit} ({', '.join(target_parts)})"
...
def get_t7_targets_and_progress(cos):
    ...
    for i, activity in enumerate(T7_ACTIVITIES):
        ...
        for m in MONTHS:
            if m in prev_months:
                count_cumulative = sum(activity_counts[activity][month] for month in MONTHS if MONTHS.index(month) <= MONTHS.index(m))
                target_cumulative = sum(t7_targets[activity][month][0] for month in MONTHS if MONTHS.index(month) <= MONTHS.index(m))
...
def get_green3_targets_and_progress(cos):
    ...
    # Define activities dynamically based on processing months
    green3_activities = {}
    if PROCESSING_MONTHS:
        for month in PROCESSING_MONTHS:
            if month == "June":
                green3_activities[month] = [{"parent": "Path Way Area", "activity": "GSB", "target": "100%"}]
            elif month == "July":
                green3_activities[month] = [{"parent": "Water Proofing - Water Body & Gazebo", "activity": "Water Proofing", "target": "100%"}]
            elif month == "August":
                green3_activities[month] = [{"parent": "Stone Work -Water Body & Gazebo", "activity": "Stone Work", "target": "100%"}]
    ...
    def find_parent_activity_row(sheet, parent_activity_name):
        ...
        search_terms = parent_variations.get(parent_activity_name, [parent_activity_name.lower()])
        search_terms.append(parent_activity_name.lower())
        ...
        for row_idx in range(1, sheet.max_row + 1):
            for col_idx in range(1, min(sheet.max_column + 1, 10)):
                ...
                if cell.value and cell.font and cell.font.bold:
                    ...
                    for search_term in search_terms:
                        if search_term in cell_text or cell_text in search_term:
                            logger.info(f"MATCH! Found parent activity '{parent_activity_name}' at row {row_idx}")
                            return row_idx, col_idx
        
        logger.warning(f"Could not find BOLD parent activity: '{parent_activity_name}'")
        return None, None
            ...
            try:
                ...
                if cell.value:
                    ...
                    if (sub_activity_name.lower() in cell_text.lower() or 
                        ...
                        logger.info(f"Found sub-activity '{sub_activity_name}' at row {search_row}")
                        ...
                        if pct_cell.value is not None:
                            try:
                                ...
                                if 0 <= val <= 100:
                                    logger.info(f"Found %Complete for '{sub_activity_name}': {val}%")
                                    return round(val, 2)
    ...
    for month in MONTHS:
        ...
        for i, act in enumerate(activities_for_month):
            ...
            if month in prev_months:
                ...
                row[f"Target achieved in {month}"] = f"{found_percent}% completed" if found_percent > 0 else "Not started"
    ...
    # Create dynamic column list for Green 3
...
def main():
    logger.info("=== STARTING VERIDIA REPORT WITH DYNAMIC MONTHS AND TRACKER SELECTION ===")
