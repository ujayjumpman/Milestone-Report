...
#         # DEBUG: Print cells that contain "EWS" or "Tower" to help find the headers
#         logger.info("\n--- DEBUG: Cells containing 'EWS' or 'Tower' in first 60 rows ---")
#         for row_idx in range(1, min(60, ws.max_row + 1)):
#             for col_idx in range(1, 20):
#                 cell_value = ws.cell(row=row_idx, column=col_idx).value
#                 if cell_value:
#                     cell_str = str(cell_value).strip().lower()
#                     if ('ews' in cell_str or 'tower' in cell_str or 'lig' in cell_str) and len(cell_str) < 50:
#                         logger.info(f"  Row {row_idx}, Col {get_column_letter(col_idx)}: '{ws.cell(row=row_idx, column=col_idx).value}'")
...
#                             logger.info(f"  ✅ {report_month}: {filename}")
#                             found = True
#                             break
                
#                 if not found:
#                     logger.warning(f"  ❌ {report_month}: Not found")
...
#             df = self._build_dataframe(section_name, activities, targets, counts, data_types, activity_tracker_months)
#             report_dfs[section_name] = df
        
#         return report_dfs
    
#     def _build_dataframe(self, section_name, activities, targets, counts, data_types, activity_tracker_months=None):
...
#             # Skip empty activities - just add blank row
#             if not name:
#                 row = {
#                     "Milestone": "",
#                     "Activity": "",
...
#         """Write Excel report"""
#         filename = f"EWS-LIG_Milestone_Report_{self.current_quarter}_{self.quarter_year}_{datetime.now():%Y%m%d}.xlsx"
...
#     def find_trackers(self):
#         """Find tracker files for EWS/LIG with month shifting: Sept from Oct, Oct from Nov, Nov from Dec"""
#         logger.info("\n=== FINDING TRACKER FILES (WITH MONTH SHIFTING) ===")
...
#             # Use None for months without tracker data
...
#         """Build milestone dataframe with cumulative logic only for available months"""
#         data = []
#         total_acts = len(activities)
#         weightage = round(100 / total_acts, 2) if total_acts else 0
...
#             else:
#                 # For non-external: only sum targets for months with trackers
...
#                 if is_external:
#                     if month_done is None:
#                         pct = 0.0
#                         month_target_pct = int(month_target) if month_target else 0
#                     else:
#                         if data_is_percentage:
#                             pct = float(month_done) if month_done else 0.0
#                             pct = min(max(pct, 0.0), 100.0)
...
#                 else:
#                     # Only accumulate for months that have trackers (not None)
...
#                         last_pct = pct
#                     else:
#                         # Month has no tracker data - show 0%
#                         row[f"% Work Done against Target-Till {month}"] = "0%"
#                         row[f"Target achieved in {month}"] = f"0 out of {int(month_target)} {unit_plural}"
